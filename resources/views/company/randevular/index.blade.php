<x-app-layout>
    ...
    <div class="card mt-3">
        <div class="card-body">
            <h5 class="card-title">
                <a href="{{ route('company.randevuexport') }}" class="btn btn-primary"><i class="fas fa-file-excel"></i> Excel</a>
            </h5>
            <table class="table table-striped">
                ...
                <tbody> 
                    @forelse ( $randevular as $randevu )
                            ...
                            @else
                                <td>{{ $randevu->TalepEdenUlke->name_en }}</td> 
                            @endif
                            ...
                            <td class="text-center">
                                @if($randevu->randevutalepeden != auth()->user()->id && $randevu->status ==2)
                                    <a href="{{ route('companyrandevu.edit',$randevu->id) }}" class="btn btn-sm btn-primary"><i class="fas fa-edit"></i></a>
                                @endif
                                <a onclick="return confirm('@lang('translate.verilersilinecek')')" href="{{ route('companyrandevu.destroy',$randevu->id) }}" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></a>
        ...
        <script>
            ...
            $('#sektor').change(function(){
                ...
                if(ulke !=''){
                    $.ajax({
                        type : 'POST',
                        url :  "{{ route('companyrandevular.firmalar') }}",
                        data:{ ulke,sektor},
           ...
           $('#company').change(function(){
               var id = this.value;
                $.ajax({
                    type : 'POST',
                    url :  "{{ route('companyrandevular.firmayetkili') }}",
                    data:{ id},
                    success:function(response){
                        if(response.count!=0){
                            $('#company_user').html('');
           ...
           $('#company_user').change(function(){
               ...
               var firma = $('#company').val();
                $.ajax({
                    type : 'POST',
                    url :  "{{ route('companyrandevular.saat') }}",
